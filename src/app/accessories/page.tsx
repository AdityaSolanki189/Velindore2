...
const Accessories: NextPage = () => {
  ...
  const getCategoryCount = (categoryName: string): number => {
    ...
    return allProducts.filter(product => product.categoryName === categoryName).length;
  };
  ...
  const sortedProducts = React.useMemo(() => {
    ...
    switch (sorting) {
      ...
      case 'popularity':
        return productsCopy.sort((a, b) => (b.discount || 0) - (a.discount || 0));
      default:
        return productsCopy;
    }
  }, [products, sorting]);
                ...
                <ul>
                  ...
                  <li className="mb-3 flex items-center">
                    ...
                    <button 
                      ...
                      className={`hover:text-gray-600 text-sm md:text-base text-left ${
                        selectedCategory === 'All Products' ? 'font-bold text-blue-600' : ''
                      }`}
                    >
                      All Products
                    </button>
                    <span className="ml-auto text-sm">({getCategoryCount('All Products')})</span>
                  </li>
                  ...
                  {categories.map((category) => (
                    <li key={category.id} className="mb-3 flex items-center">
                      ...
                      <span className="ml-auto text-sm">({getCategoryCount(category.name)})</span>
                    </li>
                  ))}
                </ul>
              </div>
              ...
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6">
                <div className="flex items-center w-full sm:w-auto mb-4 sm:mb-0">
                  ...
                  <span className="hidden sm:inline ml-4 text-sm">
                    Showing 1–{Math.min(9, sortedProducts.length)} of {sortedProducts.length} results
